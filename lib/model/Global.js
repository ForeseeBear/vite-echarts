...
function checkMissingComponents(option) {
  each(option, function (componentOption, mainType) {
    if (!ComponentModel.hasClass(mainType)) {
      ...
      if (
        componentImportName &&
        !componetsMissingLogPrinted[componentImportName]
      ) {
        error(
          "Component " +
          mainType +
...
var GlobalModel =
  ...
  (function (_super) {
    ...
    GlobalModel.prototype.init = function (
      option,
      parentModel,
      ecModel,
      theme,
    ...
    GlobalModel.prototype.setOption = function (
      option,
      opts,
    ...
    GlobalModel.prototype._mergeOption = function (newOption, opt) {
      ...
      if (replaceMergeMainTypeMap) {
        ...
        replaceMergeMainTypeMap.each(function (val, mainTypeInReplaceMerge) {
          if (
            ComponentModel.hasClass(mainTypeInReplaceMerge) &&
      ...
      function visitComponent(mainType) {
        ...
        var mergeMode = !oldCmptList // `!oldCmptList` means init. See the comment in `mappingToExists`
          ? "replaceAll"
          : replaceMergeMainTypeMap && replaceMergeMainTypeMap.get(mainType)
            ? "replaceMerge"
            : "normalMerge";
        var mappingResult = modelUtil.mappingToExists(
          oldCmptList,
          newCmptOptionList,
          mergeMode
    ...
    GlobalModel.prototype.findComponents = function (condition) {
      ...
      var result = queryCond
        ? this.queryComponents(queryCond) // Retrieve all non-empty components.
    ...
    GlobalModel.prototype.eachRawSeriesByType = function (
      subType,
      cb,
      context
    ) {
    ...
    GlobalModel.prototype.filterSeries = function (cb, context) {
      ...
      each(
        ...
        function (seriesRawIdx) {
          ...
          cb.call(context, series, seriesRawIdx) &&
...
function normalizeSetOptionInput(opts) {
  ...
  opts &&
    each(modelUtil.normalizeToArray(opts.replaceMerge), function (mainType) {
      if (process.env.NODE_ENV !== "production") {
        assert(
          ComponentModel.hasClass(mainType),
          '"' +
          mainType +
          '" is not valid component main type in "replaceMerge"'
        );
      }
