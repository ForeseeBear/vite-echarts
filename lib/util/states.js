...
export function traverseUpdateState(el, updater, commonParam) {
...
export function createEmphasisDefaultState(el, stateName, targetStates, state) {
  ...
  if (el instanceof Path) {
    ...
    if (hasFillOrStroke(fromFill) || hasFillOrStroke(fromStroke)) {
      ...
      // 线条不能添加fill
      if (el.name === 'sector3DLine') {
        emphasisStyle.fill = null
      }
...
export function createSelectDefaultState(el, stateName, state) {
...
export function createBlurDefaultState(el, stateName, state) {
