...
export function cubeView(data, dataIndex, itemModel, layout, isHorizontal, animationModel, seriesModel, isUpdate) {
    ...
    var labelPositionOutside = false ? isHorizontal ? layout.r >= layout.r0 ? 'endArc' : 'startArc' : layout.endAngle >= layout.startAngle ? 'endAngle' : 'startAngle' : isHorizontal ? layout.height >= 0 ? 'bottom' : 'top' : layout.width >= 0 ? 'right' : 'left';
    ...
    if (animationModel) {
        // 文字动画
        // animator(text, isHorizontal, animationModel, dataIndex, isUpdate);
...
export function setCubeStyle(el, data, dataIndex, itemModel, layout, seriesModel, isHorizontal) {
    // var hoverStyle = itemModel.getModel('emphasis.itemStyle').getBarItemStyle(); // 鼠标移上样式 gai
    // var color = data.getItemVisual(dataIndex, 'color');
    var color = data.getItemVisual(dataIndex, 'style').fill;//data.getItemVisual(dataIndex, 'color');
    var opacity = data.getItemVisual(dataIndex, 'opacity');
    var emphasisModel = itemModel.getModel(['emphasis']);
    var hoverStyle = {
        fill: itemModel.get(['emphasis', 'itemStyle', 'color'])
    }; // 鼠标移上样式


    var barStyle = itemModel.getModel('itemStyle')//.getBarItemStyle(); gai
